...
public class InputUtil {
	public static final InputUtil.Key UNKNOWN_KEY = InputUtil.Type.KEYSYM.createFromCode(GLFW.GLFW_KEY_UNKNOWN);
	...
	public static InputUtil.Key fromTranslationKey(String translationKey) {
		if (InputUtil.Key.KEYS_BY_ID.containsKey(translationKey)) {
			return (InputUtil.Key)InputUtil.Key.KEYS_BY_ID.get(translationKey);
		} else {
	...
	public static final class Key {
		...
		static final Map<String, InputUtil.Key> KEYS_BY_ID = Maps.newHashMap();

		Key(String translationKey, InputUtil.Type type, int keyCode) {
	...
	public static enum Type {
		KEYSYM("key.keyboard", (keyCode, translationKey) -> {
			...
			return string != null ? EmiPort.literal(string) : EmiPort.translatable(translationKey);
		}),
		SCANCODE("scancode", (scanCode, translationKey) -> {
			return EmiPort.translatable(translationKey);
		}),
		MOUSE(
			"key.mouse",
			(buttonCode, translationKey) -> EmiUtil.hasTranslation(translationKey)
					? EmiPort.translatable(translationKey)
					: EmiPort.translatable("key.mouseButton", buttonCode + 1)
		);

		private final Map<Integer, InputUtil.Key> keys = new HashMap<>();
		final String name;
		...
		public InputUtil.Key createFromCode(int code) {
			return keys.computeIfAbsent(code, (i -> {
				if (this == MOUSE) i++;
				return new InputUtil.Key(name+"."+i, this, i);
			}));
		}
