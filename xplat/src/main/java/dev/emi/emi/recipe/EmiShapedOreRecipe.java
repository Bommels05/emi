...
public class EmiShapedOreRecipe extends EmiCraftingRecipe {
    ...
    public EmiShapedOreRecipe(ShapedOreRecipe recipe) {
        super(padIngredients(recipe), EmiStack.of(recipe.getOutput()), EmiPort.getId(recipe), false);
    ...
    private static List<EmiIngredient> padIngredients(ShapedOreRecipe recipe) {
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                ...
                int height = recipe.getSize() / width;
        ...
        if (input instanceof ItemStack stack) {
            ...
            if (list.isEmpty()) {
                ...
                if (list.get(0) instanceof ItemStack) {
                    return EmiIngredient.of(streamForgeList(list).map(stack -> (ItemStack) stack).map(EmiStack::ofPotentialTag).toList());
                }
            }
        } else if (input == null) {
            return EmiStack.EMPTY;
